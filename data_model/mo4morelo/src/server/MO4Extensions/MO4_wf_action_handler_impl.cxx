...
)
{
	...
	ppcAttributeNames = (char **) MEM_alloc( sizeof(char *) * 3 );
	ppcAttributeValues = (char **) MEM_alloc( sizeof(char *) * 3 );
	...
	for ( i = 0 ; ( i < iAttachmentCount ) && ( iRetCode == ITK_ok ) ; i++ )
	{
		if(part_required_values[i] == true)
		{
			...
			if(tEngPart == NULLTAG)
			{
				...
				TC_write_syslog("\n%s = %s, %s = %s, %s = %s\n", ppcAttributeNames[0], ppcAttributeValues[0], ppcAttributeNames[1], ppcAttributeValues[1], ppcAttributeNames[2], ppcAttributeValues[2]);

				// Create Engineering Part and then relate it with Selected Engineering Design Revision
				if (!iRetCode) iRetCode = ITEM_create_item2("MO4_Eng_Part", 3, ppcAttributeNames, ppcAttributeValues, "A", &tNewEngPart, &tNewEngPartRev);
